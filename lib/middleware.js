...
var before = routeValidator._before = {};
var after = routeValidator._after = {};
...
routeValidator.addCoercer = function addCoercer (name, config) {
  switch (config.stage) {
    case 'before':
      _addCoerceMethod(before, name, config.coerce);
      break;
    case 'after':
      _addCoerceMethod(after, name, config.coerce);
      break;
    default:
  ...
  return function (req, res, next) {
    ...
    function _validatePath (path) {
      ...
      for (var key in conf) {
        ...
        for (prop in obj) {
          if (before[prop]) {
            scope[key] = before[prop](scope[key], obj[prop]);
        ...
        for (i = 0, length = tasks.after.length; i < length; i++) {
          prop = tasks.after[i];
          scope[key] = after[prop](scope[key], obj[prop]);
        }
      }
      return;
    }
